...
void game_state(int *state_p, int *buttons_p){
    /*
        ...
        button = 0 -> KEY_0 Encerra a partida
        button = 1 -> KEY_1 Seleciona modo 1 jogador
   ...
   {
    case 0:  //Menu Inicial
        if(*buttons_p == 1) *state_p = 1;  //seleciona modo single player
        else if(*buttons_p == 2) *state_p = 2; //seleciona modo dual player
    
        break;
    case 1:
        if(*buttons_p == 0) *state_p = 0;
